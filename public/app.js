...
const LEGACY_PLAYER_ID_STORAGE_KEY = "pinboard.playerId";
const initialHashPlayerPin = getHashParam("pin") ?? "";
const storedPlayerPin = localStorage.getItem(STORAGE_KEYS.playerPin) ?? "";
...
let routeTransitionInProgress = false;
const state = {
  mode: getInitialMode(),
  csrfToken: "",
  ...
  activePresentationVersion: 0,
  presentationConflict: null,
  ...
  autosaveTimer: null,
  playerId: "",
  playerPin: initialHashPlayerPin || storedPlayerPin,
  presenterEmail: "",
  presenterPassword: "",
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (action === "reload-conflicted-presentation") {
      await resolvePresentationConflict("reload");
      return;
    }
    if (action === "overwrite-conflicted-presentation") {
      await resolvePresentationConflict("overwrite");
      return;
    }
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Escape") {
    event.preventDefault();
    const presentationId = state.managementMenuId;
    state.managementMenuId = "";
    render();
    requestAnimationFrame(() => {
      document.querySelector(`[data-action="toggle-presentation-menu"][data-presentation-id="${CSS.escape(presentationId)}"]`)?.focus();
    });
    return;
  }
...
function commitRender(payload, isTransition) {
  ...
  if (payload.isImmersive) {
    ...
    if (state.mode === "presenter" && isPresenterAuthenticated() && state.activePresentationId && !state.session) {
      requestAnimationFrame(syncCreatorScrollTracking);
    }
  } else if (state.mode === "presenter" && isPresenterAuthenticated() && state.activePresentationId && !state.session) {
    requestAnimationFrame(syncCreatorScrollTracking);
  }

  if (state.mode === "presenter" && !isPresenterAuthenticated() && !state.presenterLoading) {
    requestAnimationFrame(() => {
...
function captureFocusSnapshot() {
  ...
  const snapshot = {
    ...
    presentationId: element.dataset.presentationId ?? "",
...
function restoreFocusSnapshot(snapshot) {
  ...
  const candidates = [...app.querySelectorAll(snapshot.tagName.toLowerCase())];
  const element = candidates.find((candidate) =>
    (candidate.dataset.field ?? "") === snapshot.field
    && (candidate.dataset.questionId ?? "") === snapshot.questionId
    && (candidate.dataset.optionId ?? "") === snapshot.optionId
    && (candidate.dataset.presentationId ?? "") === snapshot.presentationId
    && (candidate.dataset.action ?? "") === snapshot.action
    && (candidate.getAttribute("name") ?? "") === snapshot.name
  );
  if (!(element instanceof HTMLElement)) {
    const fallback = app.querySelector("h1, h2, main, section");
  ...
  if (
    ...
    try {
      ...
      // Inputs such as number/radio do not expose a selectable text range.
    }
  }
}
...
function patchCountValue(key, value) {
  const element = [...document.querySelectorAll("[data-count-key]")].find((candidate) => candidate.dataset.countKey === key);
  if (!element) {
    return;
  }
  const numberValue = Number(value) || 0;
  element.dataset.countValue = String(numberValue);
  element.textContent = String(numberValue);
  numberMemory.set(key, numberValue);
}

function syncEditorDraftPreview(question) {
  const title = document.querySelector(".editor-title-block h1");
  if (title) {
    title.textContent = state.draft.title || "Untitled presentation";
  }

  if (question) {
    for (const item of document.querySelectorAll("[data-question-id]")) {
      if (item.dataset.questionId !== question.id || !item.matches(".question-mini")) {
        continue;
      }
      const label = item.querySelector("strong");
      if (label) {
        label.textContent = question.text || "Untitled question";
      }
    }
    for (const radio of document.querySelectorAll('input[data-field="correctOption"]')) {
      if (radio.dataset.questionId !== question.id) {
        continue;
      }
      const optionIndex = question.options.findIndex((option) => option.id === radio.value);
      if (optionIndex >= 0) {
        radio.setAttribute(
          "aria-label",
  ...
  if (status) {
    status.textContent = getSaveStatusText();
  }
}
...
function getMotionSignature(isImmersive) {
  ...
  const role = state.mode === "presenter"
    ? state.session ? "host" : isPresenterAuthenticated() ? state.activePresentationId ? "creator" : "dashboard" : "login"
...
function renderMessages() {
  return `
    ${state.presentationConflict ? `
      <div class="error save-conflict" role="alert">
        <span>This presentation changed in another tab. Choose which version to keep.</span>
        <div class="message-actions">
          <button type="button" data-action="reload-conflicted-presentation">Reload latest</button>
          <button type="button" data-action="overwrite-conflicted-presentation">Save my draft</button>
        </div>
      </div>
    ` : state.error ? `<div class="error" role="alert"><span>${escapeHtml(state.error)}</span><button type="button" data-action="dismiss-message" aria-label="Dismiss error">&times;</button></div>` : ""}
    ${state.notice ? `<div class="notice" role="status" aria-live="polite"><span>${escapeHtml(state.notice)}</span><button type="button" data-action="dismiss-message" aria-label="Dismiss notice">&times;</button></div>` : ""}
  `;
}
      ...
      <div class="login-panel presenter-login-card stack">
        ...
        ${showLocalAuth ? `
          <form class="stack" data-action="auth">
            ...
            <label>Password
              <input type="password" autocomplete="current-password" data-field="presenterPassword" value="${escapeHtml(state.presenterPassword)}" required />
            </label>
        ...
        ${state.googleClientId ? `<div class="google-signin-slot" data-google-signin><button class="google-login-button" type="button" disabled>Loading Google sign-in...</button></div>` : ""}
      </div>
    </section>
  `;
}
  ...
  return `
    <article class="presentation-tile presentation-tile-managed">
      ...
      <div class="presentation-menu-wrap">
        <button class="presentation-menu-button" type="button" data-action="toggle-presentation-menu" data-presentation-id="${escapeHtml(presentation.id)}" aria-label="Presentation actions" aria-haspopup="menu" aria-controls="presentation-menu-${escapeHtml(presentation.id)}" aria-expanded="${menuIsOpen ? "true" : "false"}">...</button>
        ${menuIsOpen ? `
          <div class="presentation-menu" id="presentation-menu-${escapeHtml(presentation.id)}" role="menu" aria-label="Presentation actions">
...
function renderPlayer() {
  ...
  const remote = state.remote;
  if (remote.phase === "ended") {
    return renderPlayerCompletion(remote);
  }
...
function renderPlayerCompletion(remote) {
  return `
    <section class="shader-screen shader-player player-stage player-completion" data-motion-trigger="ambient-drift">
      <div class="player-ready-card">
        ...
        <h1>Game complete</h1>
        <p>You finished with <strong>${Number(remote.me?.score ?? 0).toLocaleString()}</strong> points.</p>
        <button type="button" data-action="leave-player">Leave</button>
      </div>
    </section>
  `;
}
  ...
  return `
    <section class="podium-screen">
      ...
      <div class="completion-actions">
        ...
        <button type="button" class="secondary" data-action="back-to-projects">Return to projects</button>
      </div>
    </section>
  `;
}
...
function renderJoinCodeArt(pin) {
  return `<img class="qr-image" src="/api/sessions/${encodeURIComponent(pin)}/qr.svg" alt="" aria-hidden="true" /><span class="copy-icon" aria-hidden="true"></span><strong>Copy link</strong>`;
}
...
async function loadPublicConfig() {
  try {
    ...
    state.localAuthEnabled = config.localAuthEnabled === true;
    if (state.mode === "presenter" && !isPresenterAuthenticated()) {
      render();
    }
  } catch {
    state.googleClientId = "";
    state.localAuthEnabled = false;
  }
}
...
function acceptPresenterSession(presenter, csrfToken) {
  if (!presenter || typeof csrfToken !== "string" || !csrfToken) {
    throw new Error("The presenter session could not be established.");
  }
...
async function restorePresenterIfPossible() {
  ...
  try {
    ...
    if (pendingPresentationId) {
      state.pendingPresentationId = pendingPresentationId;
      sessionStorage.setItem(STORAGE_KEYS.pendingPresentationId, pendingPresentationId);
    }
...
async function openPendingPresentationIfNeeded() {
  ...
  if (!presentationId || !isPresenterAuthenticated()) {
    return false;
  }
  ...
  try {
    ...
    state.activePresentationVersion = 0;
    state.lastSavedAt = "";
    state.draft = createDraft();
    state.activeQuestionId = state.draft.questions[0]?.id ?? "";
    resetDraftSaveCoordinator();
    updateBrowserUrl(PRESENTATION_HOME_PATH, { replace: true });
    showError(error);
    return false;
  }
...
async function openPresentationRecord(presentation, options = {}) {
  ...
  state.activePresentationVersion = Number(presentation.version ?? 1);
...
async function resolvePresentationConflict(strategy) {
  const presentationId = state.presentationConflict?.presentationId;
  if (!presentationId || presentationId !== state.activePresentationId) {
    return;
  }
  const latest = (await getJson(`/api/presentations/${encodeURIComponent(presentationId)}`, true)).presentation;
  if (strategy === "reload") {
    state.presentationConflict = null;
    await openPresentationRecord(latest, { updateUrl: false });
    showNotice("Reloaded the latest saved version.");
    return;
  }
  ...
  try {
    const response = await putJson(`/api/presentations/${encodeURIComponent(presentationId)}`, {
      snapshot: serializeDraftForSave(),
      expectedVersion: Number(latest.version)
    }, true);
    state.draft = clonePresentationSnapshot(response.presentation.snapshot);
    state.activePresentationUpdatedAt = response.presentation.updatedAt ?? "";
    state.activePresentationVersion = Number(response.presentation.version);
    state.lastSavedAt = response.presentation.updatedAt ?? new Date().toISOString();
    state.presentationConflict = null;
    upsertPresentationSummary(response.presentation);
    resetDraftSaveCoordinator();
    showNotice("Saved this draft over the newer version.");
    render();
  } catch (error) {
    if (error?.code === "PRESENTATION_VERSION_CONFLICT") {
      state.presentationConflict = { presentationId };
      syncMessages();
    }
    throw error;
  }
...
async function backToProjects() {
  if (state.presentationConflict) {
    if (!window.confirm("This draft conflicts with a newer saved version. Leave and discard this tab's unsaved draft?")) {
      return;
    }
    resetDraftSaveCoordinator();
  } else if (state.presentationDirty) {
...
function togglePresentationMenu(presentationId) {
  const opening = state.managementMenuId !== presentationId;
  state.managementMenuId = opening ? presentationId ?? "" : "";
  render();
  if (opening) {
    requestAnimationFrame(() => {
      document.querySelector(`#presentation-menu-${CSS.escape(presentationId)} [role="menuitem"]`)?.focus();
    });
  }
}
...
async function renamePresentation(presentationId) {
  ...
  const updated = await putJson(`/api/presentations/${encodeURIComponent(presentationId)}`, {
    snapshot,
    expectedVersion: Number(response.presentation?.version ?? 1)
  }, true);
  if (updated.presentation) {
    ...
    if (state.activePresentationId === presentationId) {
      ...
      state.activePresentationVersion = Number(updated.presentation.version ?? state.activePresentationVersion);
...
function createPresentationSaveCoordinator() {
  return createDraftSaveCoordinator({
    createSnapshot: () => ({
      presentationId: state.activePresentationId,
      expectedVersion: state.activePresentationVersion,
      payload: serializeDraftForSave()
    }),
    saveSnapshot: async ({ snapshot }) => {
      if (!snapshot.presentationId) {
        throw new Error("Choose a presentation before saving.");
      }

      let response;
      try {
        response = await putJson(
          `/api/presentations/${encodeURIComponent(snapshot.presentationId)}`,
          { snapshot: snapshot.payload, expectedVersion: snapshot.expectedVersion },
          true
        );
      } catch (error) {
        if (error?.code === "PRESENTATION_VERSION_CONFLICT" && state.activePresentationId === snapshot.presentationId) {
          state.presentationConflict = { presentationId: snapshot.presentationId };
          state.error = "";
          syncMessages();
        }
        throw error;
      }
      if (response.presentation?.id !== snapshot.presentationId) {
        throw new Error("The saved presentation response did not match the requested presentation.");
      }

      upsertPresentationSummary(response.presentation);
      if (state.activePresentationId === snapshot.presentationId) {
        state.presentationConflict = null;
        state.activePresentationUpdatedAt = response.presentation.updatedAt ?? "";
        state.activePresentationVersion = Number(response.presentation.version ?? snapshot.expectedVersion + 1);
        state.lastSavedAt = response.presentation.updatedAt ?? new Date().toISOString();
      }
    }
  });
...
function resetDraftSaveCoordinator() {
  state.presentationConflict = null;
  draftSaveCoordinator = createPresentationSaveCoordinator();
  syncDraftSaveState();
}

function syncDraftSaveState() {
  const saveState = draftSaveCoordinator.getState();
  state.presentationDirty = saveState.dirty;
  state.savingPresentation = saveState.saving;
}
...
async function createSession() {
  ...
  connectEvents(response.pin, "host");
  render();
}
...
async function restorePlayerIfPossible() {
  ...
  try {
    const legacyPlayerId = localStorage.getItem(LEGACY_PLAYER_ID_STORAGE_KEY) ?? "";
    const response = await postJson(`/api/sessions/${pin}/resume`, legacyPlayerId ? { legacyPlayerId } : {});
    ...
    state.playerId = "";
    state.restoreKey = "";
    if (shouldRetainResumeCredential(error)) {
      state.notice = "The live session could not reconnect yet. Your Game PIN has been kept so you can retry.";
      render();
      return;
    }
...
async function hostAction(action) {
  ...
  const payload = action === "end" ? { discardActiveRound: true } : {};
  if (action === "end" && !window.confirm("End this live session? The active round will be discarded and players will be sent to the final results.")) {
    return;
  }

  const response = await postJson(`/api/sessions/${state.remote.pin}/${action}`, payload, true);
  applyRemoteState(response.session);
  render();
}
...
async function submitAnswer(optionId) {
  ...
  if (response.session) {
    ...
    applyCompactAnswerState(response);
  }
  render();
}

async function postJson(url, payload, presenterRequest = false) {
  return requestJson("POST", url, payload, presenterRequest);
}

async function putJson(url, payload, presenterRequest = false) {
  return requestJson("PUT", url, payload, presenterRequest);
}

async function getJson(url, presenterRequest = false) {
  return requestJson("GET", url, null, presenterRequest);
}

async function deleteJson(url, presenterRequest = false) {
  return requestJson("DELETE", url, null, presenterRequest);
}

async function requestJson(method, url, payload, presenterRequest = false) {
  const headers = { Accept: "application/json" };
  if (payload !== null) {
    headers["Content-Type"] = "application/json";
  }
  if (presenterRequest && isStateChangingMethod(method)) {
    if (!state.csrfToken) {
      throw new Error("The presenter session has expired. Please sign in again.");
    }
  ...
  if (presenterRequest && typeof body.csrfToken === "string" && body.csrfToken) {
...
function connectEvents(pin, role) {
  if (state.eventSource) {
    state.eventSource.close();
  }
...
function updateFromInput(target) {
  ...
  syncEditorDraftPreview(question);
}

function applyCompactAnswerState(answer) {
  if (!state.remote || answer.pin && answer.pin !== state.remote.pin) {
    return;
  }
  state.remote = {
    ...state.remote,
...
function applyRemoteState(nextState) {
  if (!shouldAcceptLiveState(state.remote, nextState)) {
    return false;
  }
...
async function attachMedia(target) {
  ...
  if (state.activePresentationId !== presentationId || !currentQuestion) {
    if (response.media?.id) {
      await deleteJson(`/api/media/${encodeURIComponent(response.media.id)}`, true).catch(() => undefined);
    }
    throw new Error("The upload finished after you left that question, so it was safely discarded.");
  }
  currentQuestion.media = response.media;
  markPresentationDirty();
  render();
}
...
async function uploadMedia(file) {
  ...
  const body = await response.json().catch(() => ({}));
  if (!response.ok) {
    const uploadError = new Error(body.error ?? "Media upload failed.");
    uploadError.status = response.status;
    uploadError.code = body.code;
    throw uploadError;
  }
  if (!body.media?.url) {
    throw new Error("Media upload did not return a usable file.");
  }
...
function setMode(mode) {
  ...
  if (mode === "home") {
    ...
    updateBrowserUrl(isPresenterAuthenticated() ? PRESENTATION_HOME_PATH : PRESENTATION_LOGIN_PATH);
  }
  render();
}

async function signOutPresenter() {
  if (state.presentationDirty) {
    await savePresentation({ silent: true });
  }
  ...
  try {
    if (isPresenterAuthenticated()) {
      await postJson("/api/logout", {}, true);
    }
  } catch (error) {
    if (error.status !== 401) {
      throw error;
...
function syncAutosaveTimer() {
  const shouldAutosave = state.mode === "presenter" && isPresenterAuthenticated() && state.activePresentationId && !state.session;
...
function isPresenterAuthenticated() {
  return Boolean(state.presenter);
}
...
function createOptions(labels) {
  return labels.map((text) => ({ id: createClientId(), text: limitClientText(text, MAX_OPTION_TEXT_LENGTH) }));
}
...
async function syncRouteStateFromLocation() {
  if (routeTransitionInProgress) {
    return;
  }
  const route = getLocationRoute();
  const nextMode = route.mode || getInitialMode();
  const leavesDirtyPresentation = state.presentationDirty
    && state.activePresentationId
    && (nextMode !== "presenter" || route.presentationId !== state.activePresentationId);
  if (leavesDirtyPresentation) {
    if (state.presentationConflict) {
      const discard = window.confirm("This draft conflicts with a newer saved version. Leave and discard this tab's unsaved draft?");
      if (!discard) {
        updateBrowserUrl(getPresentationPath(state.activePresentationId), { replace: true });
        return;
      }
      resetDraftSaveCoordinator();
    } else {
  ...
  if (nextMode === "presenter" && route.presentationId && !isPresenterAuthenticated()) {
...
function unlockPresenterMusic() {
  if (state.mode !== "presenter" || !isPresenterAuthenticated() || !state.presenterMusic.enabled) {
    return;
  }
...
function getPresenterMusicMode() {
  if (state.mode !== "presenter" || !isPresenterAuthenticated() || !state.session || !state.remote) {
    return "";
  }
...
function showError(error) {
  state.error = error?.code === "PRESENTATION_VERSION_CONFLICT"
    ? ""
